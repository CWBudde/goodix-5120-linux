...
func TestReplayHappyPath(t *testing.T) {
	script := []Exchange{
		{Cmd: opFWVer, Payload: nil, Response: []byte{0xa0, 0x01, 0x02}},
		{Cmd: opNOP, Payload: []byte{0x00, 0x00}, Response: []byte{0xb0}},
	}
	...
	if !bytes.Equal(got, script[0].Response) {
		t.Errorf("Recv = %x, want %x", got, script[0].Response)
	}

	if err := tr.Send(opNOP, []byte{0x00, 0x00}); err != nil {
		t.Fatalf("Send(nop): %v", err)
	}
	if got, err = tr.Recv(0); err != nil || !bytes.Equal(got, script[1].Response) {
		t.Fatalf("Recv = %x, %v; want %x, nil", got, err, script[1].Response)
	}
...
func TestReplayEnforcesSameCeiling(t *testing.T) {
	script := []Exchange{{Cmd: opReset, Response: []byte{0x01}}}
	...
	if _, err := tr.Recv(0); err == nil {
		t.Error("Recv succeeded after a refused Send; no response should have been queued")
	}
}
...
func TestReplayNilPayloadSkipsCheck(t *testing.T) {
	tr := NewReplay([]Exchange{{Cmd: opNOP, Payload: nil, Response: []byte{0x09}}}, Options{})
	defer tr.Close()
